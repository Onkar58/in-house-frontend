...
const Profile = () => {
  ...
  const getTeachersData = async () => {
    const teachersData = await _axios.post(`/user/getuserbyemail/`, {
      email: user?.email,
    });
    if (teachersData.success) {
      setTeachersData(teachersData.user);
    } else {
  ...
  const addBatch = async (e) => {
    e.preventDefault();
    const isBatchAdded = await _axios.post(`/user/addbatch/`, {
      email: user.email,
      formData: batchData,
    });
    if (isBatchAdded.success) {
      toast.success(isBatchAdded.message);
      setIsFormOpen(false);
    }
  };

  const deleteBatch = async () => {
    const isBatchDeleted = await _axios.post(`/user/deletebatch/`, {
      email: user.email,
    });
    if (isBatchDeleted.success) {
      toast.success(isBatchDeleted.message);
    }
  };
    ...
    <>
      ...
      {teachersData?.batches?.length === 0 ? (
        <button
          ...
          className="bg-green-400 bg-opacity-80 p-3 px-5 rounded-md text-white font-[600] cursor-pointer"
        >
          Add TG Batch
        </button>
      ) : (
        <section>
          ...
          {teachersData &&
            teachersData?.batches?.map((batch, idx) => (
              <p
                className="text-white bg-[#d9d9d9] bg-opacity-10 flex items-center justify-start p-2 text-xl"
                key={idx}
              >
                ...
                <span className="w-1/4 text-center">
                  {batch.year.toUpperCase()}
                </span>
                <span className="w-1/4 text-center">
                  {batch.div.toUpperCase()}
                </span>
                <span className="w-1/4 text-center">
                  {batch.startNo} - {batch.endNo}
                </span>
                <span
                  className="ml-auto"
                  title="Delete Batch"
      ...
      {isFormOpen && (
        <div
          ...
          className="absolute h-full w-full bg-black  bg-opacity-50 top-0 left-0 flex items-center justify-center backdrop-blur-md"
        >
          <form
            onSubmit={addBatch}
            onClick={(e) => e.stopPropagation()}
            className="flex flex-col gap-2 blur-none "
          >
            <input
              className={inputStyles}
              type="text"
              value={batchData.branch}
              onChange={changeBatchData}
              name="branch"
              placeholder="TG Branch"
            />
            <input
              className={inputStyles}
              type="text"
            ...
            <input
              ...
              onChange={changeBatchData}
              name="div"
              placeholder="TG Division"
            />
            <input
              className={inputStyles}
              type="number"
              value={batchData.startNo}
              onChange={changeBatchData}
              name="startNo"
