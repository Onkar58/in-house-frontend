...
const StudentCard = ({ data, email, deleteUser }) => {
  const navigate = useNavigate()
  return (
    <div className="relative size-32 sm:size-40 md:size-48" onClick={() => navigate(`/student/${data.username}`)}>
      <svg className='size-10 absolute -top-10 -right-10 bg-white p-2 rounded-full cursor-pointer' viewBox="0 0 24 24" onClick={(e) => {deleteUser(email, data.username); e.stopPropagation()}}>
