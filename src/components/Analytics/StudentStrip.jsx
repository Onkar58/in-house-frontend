...
const StudentStrip = ({ submissionData, data }) => {
  let status;
  const submissionTime = parseFloat(
    submissionData?.recentSubmissionList[0]?.timestamp,
  );
  if (
    parseInt(Date.now() / 1000) - submissionTime <= 604800 &&
  ...
  return (
    <div className="bg-[#d9d9d9] text-white bg-opacity-10 flex items-center justify-between w-full px-10 py-3 rounded-lg">
      <p
        className="underline text-lg min-w-[200px] cursor-pointer"
        onClick={() => navigate(`/student/${data.username}`)}
      >
        {data.username}
      </p>
